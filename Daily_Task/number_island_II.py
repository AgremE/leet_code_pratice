...
class Solution:
    def numIslands2(self, m: int, n: int, positions: List[List[int]]) -> List[int]:
        ...
        def get_conneted_land(grid,pos):
            ...
            connected_node = [(x,y) for x,y in connected_node if (x>=0 and x<m) and (y>=0 and y < n)]
        ...
        for node in positions:
            ...
            cluster,tracker,cur_cluster_id = update_cluster(cluster,tracker,grid,node,cur_cluster_id)
            result.append(len(cluster))
        return result

solution = Solution()
print(solution.numIslands2(3,3,[[0,0],[0,1],[1,2],[2,1],[1,0],[0,0],[2,2],[1,2],[1,1],[0,1]]))