        ...
        for meeting in meetings:
            ...
            if next_meeting_removal:
                ...
                if len(next_meeting_removal) < n:
                    n_m = heapq.heappop(
                        next_meeting_removal
                    )  # n_m is [end_time,room_n]
                    if (n_m[0] - n_m[1]) <= s_t * (10**9):
                        heapq.heappush(
                            next_meeting_removal, (e_t * 10**9 + n_m[1], n_m[1])
                        )
                        keep_count[n_m[1]] += 1
                    else:
                        heapq.heappush(next_meeting_removal, n_m)
                        heapq.heappush(
                            next_meeting_removal, (e_t * 10**9 + cur_room, cur_room)
                        )
...
solution = Solution()
print(
    solution.mostBooked(n=4, meetings=[[18, 19], [3, 12], [17, 19], [2, 13], [7, 10]])
)
